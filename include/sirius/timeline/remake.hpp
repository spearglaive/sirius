...
namespace acma::timeline::impl {
	...
	constexpr result<void> remake(
		...
		if constexpr(config.access != access_policy::exclusive && !PreserveOldAllocation && !CopyData){
			...
			constexpr bool can_destroy_old_alloc_next_frame = (config.access == access_policy::exclusive && !PreserveOldAllocation && !CopyData);
