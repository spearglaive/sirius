...
int main(){
	RESULT_OUTPUT(acma::intitialize_lib("Sirius Test", acma::version{1,0,0}));
    ...
    sl::unique_ptr<render_instance> inst = *std::move(inst_result);


	//llfio::mapped_file_handle mh;
	//RESULT_TRY_MOVE(mh, acma::decoder::open_file(assets_path / "test_font.ttf"));
	//RESULT_OUTPUT(acma::decoder::decode_font(mh));
	...
	{
	RESULT_OUTPUT((sl::universal::get<buffer_id::dispatch_commands>(*inst).template try_emplace_back<VkDispatchIndirectCommand>(
		1, 1, 1
	)));
	}
	...
	{
	RESULT_OUTPUT(sl::universal::get<buffer_id::draw_commands>(*inst).reserve(sizeof(acma::indexed_draw_command_t))); //Should do nothing
	RESULT_OUTPUT((sl::universal::get<buffer_id::draw_commands>(*inst).template try_emplace_back<acma::indexed_draw_command_t>(
		6, 3, 0, 0, 0
	)));
	RESULT_OUTPUT((sl::universal::get<buffer_id::counts>(*inst).template try_emplace_back<acma::draw_count_t>(
		1
	)));


	RESULT_OUTPUT((staging_buffer.template emplace_back<acma::indexed_draw_command_t>(
		6, 1, 0, 0, 0
	)));

	RESULT_OUTPUT(sl::universal::get<buffer_id::single_instance_draw_command>(*inst).resize(sizeof(acma::indexed_draw_command_t)));
	RESULT_OUTPUT((acma::gpu_copy(
		*inst,
	...
	{
	RESULT_OUTPUT(staging_buffer.resize(rect_indices.size_bytes()));
	std::memcpy(staging_buffer.data(), rect_indices.data(), rect_indices.size_bytes());


	RESULT_OUTPUT(staging_buffer.upload_to(sl::universal::get<buffer_id::rectangle_indices>(*inst)));

	//Equivalent to:
	// RESULT_OUTPUT(sl::universal::get<buffer_id::rectangle_indices>(*inst).resize(rect_indices.size_bytes()));

	// RESULT_OUTPUT((acma::copy(
	...
	{
	RESULT_OUTPUT(staging_buffer.resize(rect_positions.size_bytes()));
	std::memcpy(staging_buffer.data(), rect_positions.data(), rect_positions.size_bytes());

	RESULT_OUTPUT(sl::universal::get<buffer_id::positions>(*inst).resize((sizeof(acma::pt2u32) * 16 * 16) + 1));
	...
	//RESULT_OUTPUT((sl::universal::get<buffer_id::counts>(*inst).try_push_back(
	//	rect_limit
	//)));
	RESULT_OUTPUT((staging_buffer.try_push_back<sl::uint32_t>(
		rect_limit
	)));
	RESULT_OUTPUT(staging_buffer.try_upload_to(sl::universal::get<buffer_id::counts>(*inst), sizeof(acma::draw_count_t)));
	//Equivalent to:
	// RESULT_OUTPUT(sl::universal::get<buffer_id::counts>(*inst).try_resize(sizeof(sl::uint32_t) + sizeof(acma::draw_count_t)));
	// RESULT_OUTPUT((acma::copy(
	...
	RESULT_OUTPUT(sl::universal::get<buffer_id::offset>(*inst).resize(sizeof(decltype(offset))));
	RESULT_OUTPUT((acma::gpu_copy(
		*inst,
